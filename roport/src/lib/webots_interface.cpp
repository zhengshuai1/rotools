...
namespace roport
{
  WalkerWebotsInterface::WalkerWebotsInterface(ros::NodeHandle& nh, std::vector<std::string> joint_names) :
    nh_(nh),
  ...
  WalkerWebotsInterface::~WalkerWebotsInterface() = default;

  void WalkerWebotsInterface::init()
  {
    headStateSub = nh_.subscribe("/walker/head/joint_states", 1, &WalkerWebotsInterface::headStateCallback, this);
    leftHandStateSub = nh_.subscribe("/walker/leftHand/joint_states", 1, &WalkerWebotsInterface::leftHandStateCallback, this);
    leftLimbStateSub = nh_.subscribe("/walker/leftLimb/joint_states", 1, &WalkerWebotsInterface::leftLimbStateCallback, this);
    rightHandStateSub = nh_.subscribe("/walker/rightHand/joint_states", 1, &WalkerWebotsInterface::rightHandStateCallback, this);
    rightLimbStateSub = nh_.subscribe("/walker/rightLimb/joint_states", 1, &WalkerWebotsInterface::rightLimbStateCallback, this);
    legsStateSub = nh_.subscribe("/walker/Leg/MeasuredJoint", 1, &WalkerWebotsInterface::legsStateCallback, this);
    ...
    // Resize vectors
    currentJointPosition.resize(joint_names_.size());
    currentJointVelocity.resize(joint_names_.size());
    currentJointEffort.resize(joint_names_.size());
    jointPositionCommand.resize(joint_names_.size());
    jointVelocityCommand.resize(joint_names_.size());
    jointEffortCommand.resize(joint_names_.size());
    ...
    std::fill(jointVelocityCommand.begin(), jointVelocityCommand.end(), 0);
    std::fill(jointEffortCommand.begin(), jointEffortCommand.end(), 0);
    ROS_INFO_STREAM("Initialized buffers");
  }

  void WalkerWebotsInterface::read()
  {
    for (int i = 0; i < joint_names_.size(); i++) {
      try {
        //ROS_INFO_STREAM("Getting state of joint "<<joint_names_[i]);
        std::unique_lock<std::timed_mutex> lock(jointStatesMutex, std::chrono::seconds(10));
        if (lock) {
          std::vector<double> jointState = jointStates.at(joint_names_[i]);
        ...
        //ROS_INFO_STREAM("Got state of joint "<<joint_names_[i]);
      }
  ...
  void WalkerWebotsInterface::write()
  {
  ...
  double WalkerWebotsInterface::getJointCommandedValue(const std::string& jointName)
  {
    for (int i = 0; i < joint_names_.size(); i++) {
      if (joint_names_[i] == jointName)
        return jointPositionCommand[i];
    }
    return 0.0;
  }

  void WalkerWebotsInterface::setJointsStates(const sensor_msgs::JointState::ConstPtr& msg,
                                              const std::vector<std::string>& joint_names)
  {
    /*
    std::string js = "";
    for(std::string j : joint_names_)
      js+=j;
    ROS_INFO_STREAM("Setting state of joints "<<js);
    */
    assert((msg->position.size() != joint_names.size() || msg->velocity.size() != joint_names.size() ||
            msg->effort.size() != joint_names.size(),"Mismatch in the number of joints"));
    ...
    for (int i=0; i < joint_names.size(); i++)
      jointStates[joint_names[i]] = std::vector<double>({msg->position.at(i), msg->velocity.at(i), msg->effort.at(i)});
  }

  void WalkerWebotsInterface::headStateCallback(const sensor_msgs::JointState::ConstPtr& msg)
  {
    setJointsStates(msg,std::vector<std::string>({"head_j1",    // HeadYaw
                                                  "head_j2"})); // HeadPitch
  }

  void WalkerWebotsInterface::leftHandStateCallback(const sensor_msgs::JointState::ConstPtr& msg)
  {
  ...
  void WalkerWebotsInterface::leftLimbStateCallback(const sensor_msgs::JointState::ConstPtr& msg)
  {
  ...
  void WalkerWebotsInterface::rightHandStateCallback(const sensor_msgs::JointState::ConstPtr& msg)
  {
  ...
  void WalkerWebotsInterface::rightLimbStateCallback(const sensor_msgs::JointState::ConstPtr& msg)
  {
  ...
  void WalkerWebotsInterface::legsStateCallback(const sensor_msgs::JointState::ConstPtr& msg)
  {
  ...
  void WalkerWebotsInterface::sendLeftLimbPositionCommand(double shoulderPitch, double shoulderRoll, double shoulderYaw, double elbowRoll, double elbowYaw, double wristPitch, double wristRoll)
  {
  ...
  void WalkerWebotsInterface::sendRightLimbPositionCommand(double shoulderPitch, double shoulderRoll, double shoulderYaw, double elbowRoll, double elbowYaw, double wristPitch, double wristRoll)
  {
    ubt_core_msgs::JointCommand msg;
    msg.mode = 5;
    msg.command = std::vector<double>({shoulderPitch, shoulderRoll, shoulderYaw, elbowRoll, elbowYaw, wristPitch, wristRoll});
  ...
  void WalkerWebotsInterface::sendHeadPositionCommand(double pitch, double yaw)
  {
    ubt_core_msgs::JointCommand msg;
    msg.mode = 5;
  ...
  void WalkerWebotsInterface::sendLegsPositionCommand(double leftHipYaw,
                                                      double leftHipRoll,
                                                      double leftHipPitch,
